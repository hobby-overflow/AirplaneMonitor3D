    ...
    public genUrl():string[][] {
        ...
        for (let y = 0; y < tbl.length; y++) {
            for (let x = 0; x < tbl[y].length; x++) {
                tbl[y][x] = `https://api.mapbox.com/styles/v1/mapbox/satellite-v9/tiles/${zoomLevel}/${tileX-this.centerRow+x}/${tileY-this.centerCol+y}@2x?access_token=${api_id}`;
            }
        }
        ...
        this.mostSW = (_ => {
            ...
            const lat_lad = Math.atan(Math.sinh(Math.PI * (1 - 2 * this.maxTileNum[1] / n )));
        ...
        this.mostNE = (_ => {
            ...
            const lat_lad = Math.atan(Math.sinh(Math.PI * (1 - 2 * this.minTileNum[1] / n )));
            const lat_deg = lat_lad * ( 180 / Math.PI);
